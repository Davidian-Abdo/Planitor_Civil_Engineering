...
def generate_tasks(
    ...
    """
    Generate tasks for all zones/floors based on base_tasks_dict.
    
    Features:
    - Preserves ground, special (4.5,4.6,4.7), and regular task logic
    - Cross-floor dependencies preserved
    - Cross-zone dependencies added based on per-discipline zone configuration
    - discipline_zone_cfg: { discipline: DisciplineZoneConfig }
    """
    ...
    # ---------- Step 2: Generate tasks with dependencies ----------
    for discipline, base_list in base_tasks_dict.items():
        for base in base_list:
            if not getattr(base, "included", True):
                continue
            ...
            # Determine zone groups for this discipline
            if cfg and cfg.zone_groups:
                zone_groups = cfg.zone_groups
                strategy_type = cfg.strategy
            else:
                # fallback: all zones in a single parallel group
            ...
            for group_idx, zone_group in enumerate(zone_groups):
                for zone in zone_group:
                    ...
                    # Determine floor range for this task
                    if base.discipline in ground_disciplines:
                        floor_range = [0]
                    elif is_special:
                        floor_range = range(max_floor + 1) if getattr(base, "repeat_on_floor", True) else [0]
                    ...
                    for f in floor_range:
                        ...
                        if is_special and f == 0:
                            ...
                            for p in base.predecessors:
                                pred_base = base_by_id.get(p)
                                if pred_base and getattr(pred_base, "included", True):
                                    pred_floor = 0 if pred_base.discipline in ground_disciplines else f
                                    pred_id = f"{p}-F{pred_floor}-{zone}"
                                    if pred_id in task_ids:
                                        preds.append(pred_id)

                            # ---------- Cross-floor predecessors ----------
                            if f > 0:
                                # same task previous floor
                                prev_floor_task = f"{base.id}-F{f-1}-{zone}"
                                if prev_floor_task in task_ids:
                                    preds.append(prev_floor_task)
                                # additional cross-floor links
                            ...
                            # ---------- Cross-zone dependencies (per discipline) ----------
                            if group_idx > 0:
                                # sequential groups: depend on all zones in previous groups
                                ...
                                # fully_parallel or first group: no cross-zone deps
                                pass
